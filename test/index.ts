...
import {PrivateKey, Client, utils, Signature} from 'ddpayv1'
...
const ddpayVerify: VerifyMessage = async (message: Buffer, signatures: string[], account: string) => {
    const opts = {
        hash: message,
        signatures,
...
describe('rpc auth', function() {
    ...
    it('handles invalid signatures', async function() {
        ...
        // valid
        await validate(signed, ddpayVerify)

        // invalid method
        invalid = utils.copy(signed)
        invalid.method = 'foo.bar2'
        error = await assertThrows(async () => {
            await validate(invalid, ddpayVerify)
        })
